...
def extrapolate_data(out_path: str, x_extrap: int, df_name: str):
    ...
    df_extrap_mask = pd.read_csv(os.path.join(out_path, f"{df_name}_interpolated_mask.csv"),index_col=0)
    ...
    print(f"Saving to csv in {out_path + f'/{df_name}_extrapolated_' + str(x_extrap) + '.csv'}")
    print(f"Saving to csv in {out_path + f'/{df_name}_extrapolated_mask_' + str(x_extrap) + '.csv'}")

    df.to_csv(os.path.join(out_path, f"{df_name}_extrapolated_" + str(x_extrap) + ".csv"))
    df_extrap_mask.to_csv(os.path.join(out_path, f"{df_name}_extrapolated_mask_" + str(x_extrap) + ".csv"))
